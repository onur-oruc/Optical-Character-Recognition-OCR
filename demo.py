...
def process_ocr(folder_path_="."):
    ...
    for file in os.listdir(folder_path_):
        if file.endswith(tuple(image_format)):
            ...
            img = Image.open(os.path.join(folder_path_, file))
            img = np.array(img)
            st.image(img)
    ...
    file = os.path.join(folder_path_, "meta_data.json")
    print(os.getcwd())
    with open(file, "w") as jsonfile:
        json.dump(json_object, jsonfile)
        st.write("json created")
...
directory_options = ["Current Directory", "Change Directory"]
...
if directory_option.title() == directory_options[0]:  # current directory
    folder_path = '.'
    st.write('Selected directory: `%s`' % folder_path)
